...
export const skillService = {
  ...
  search: (query: string, page = 0, size = 10, category?: string, excludeUserId?: number, signal?: AbortSignal) => {
    const params = new URLSearchParams({ page: String(page), size: String(size) });
    if (query) params.set('search', query);
    if (category) params.set('category', category);
    if (excludeUserId) params.set('excludeUserId', String(excludeUserId));
    return apiClient.get<PageResponse<SkillDetailResponse>>(`/api/v1/skills?${params}`, { signal }).then((r) => r.data);
  },
